...
const Inventory = () => {
  ...
  const handleAddItem = async () => {
    ...
    try {
      await addInventoryItem({ name: newItem.name, sku: newItem.sku, price: p, stock: s });
    ...
    <div className="animate-fade">
      <header className="page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <ModuleTutorial title="Inventario" description="Controla tus activos y productos." steps={tutorialSteps} />
        </div>
        <div className="header-actions">
          <div className="search-bar-wrapper">
            <Search className="search-bar-icon" size={18} />
            <input type="text" placeholder="Buscar producto o SKU..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
            ...
            <tbody>
              {filteredInventory.map((item: any) => (
                <tr key={item._id}>
                  ...
                  <td><span style={{ fontWeight: 700 }}>${(item.price || 0).toLocaleString()}</span></td>
                  <td>{item.stock}</td>
                  <td><span className={`status-badge ${item.status}`}>{item.status}</span></td>
                  <td style={{ textAlign: 'right' }}>
                    <button className="btn-secondary" style={{ padding: '0.4rem', color: 'var(--error)', borderColor: 'transparent' }} onClick={() => deleteInventoryItem(item._id)}>
                      <Trash2 size={16} />
                    </button>
                  </td>
                </tr>
