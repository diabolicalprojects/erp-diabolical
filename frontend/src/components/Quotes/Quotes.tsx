...
const Quotes = () => {
    ...
    const filteredQuotes = (quotes || []).filter(q =>
        ...
        (q.folio || q.id || '')?.toLowerCase().includes(searchTerm.toLowerCase())
    );
                    ...
                    <table className="data-table">
                        ...
                        <tbody>
                            {filteredQuotes.map((q) => {
                                ...
                                return (
                                    <tr key={q._id || q.id}>
                                        <td style={{ color: 'var(--text-primary)', fontWeight: 600 }}>{q.folio || q.id}</td>
                                        <td>
                                            {q.type === 'custom'
                                        ...
                                        <td>{q.customer}</td>
                                        <td><span style={{ opacity: 0.6 }}>{q.date ? new Date(q.date).toLocaleDateString() : ''}</span></td>
                                        <td><span style={{ fontWeight: 700 }}>${(q.amount || 0).toLocaleString()}</span></td>
                                        <td>
                                        ...
                                        <td style={{ textAlign: 'right' }}>
                                            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                                                ...
                                                <button className="btn-secondary" style={{ padding: '0.4rem', color: 'var(--error)', borderColor: 'transparent' }} onClick={() => deleteQuote(q._id || q.id)}>
                                                    <Trash2 size={16} />
                                                </button>
                                            </div>
                                        </td>
